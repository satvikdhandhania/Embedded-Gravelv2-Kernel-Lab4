The user program dagger.c initializes function name, data, stack position of user program,  frequency of device, and then task_create sys call is called.

In task_create we check if number of talks are less than 63, and user has initialized valid stack addresses within bounds of user stack start and end addresses, we then sort the input task, and initialize the sleep queues to be null and match values to be current system time plus device frequency of each task, we then schedule idle task and clear the run list and sets them to empty. The user tasks are then initialized to system tcb, sending function in r4, data in r5, stack position in r6 and kernel stack in sp,lr is given launch task. After initializing tcb we add them to run queue by calling runqueue_add, where group and position in group are found and added to group_run_bits and run_bits.
...
When user calls, mutex_lock, we disable interrupt then check for the availiabilty, if available, and if gtMutex[mutex].bLock is not locked already by some other task, the task is assigned the mutex, if blocked then the task is added to mutex sleep queue where it waits for mutex to be unlocked, dispatch sleep is called then. In case of mutex_unlock, we remove the other tasks of that mutex from sleep queue, and add it to run queue is then called.
